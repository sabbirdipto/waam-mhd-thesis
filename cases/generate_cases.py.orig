...
def write_decompose_par_dict(case_dir, values):
    """Fully determined by two CSV columns, so generate it rather than template it."""
    n = int(float(values.get("n_procs", 6)))
    method = (values.get("decomp_method") or "scotch").strip()
    (case_dir / "system").mkdir(parents=True, exist_ok=True)
    (case_dir / "system" / "decomposeParDict").write_text(f"""\
/*--------------------------------*- C++ -*----------------------------------*\\
...
// scotch partitions by graph connectivity, so it copes with graded meshes and
// with directions only one cell thick, where 'simple' fails outright.
method              {method};
...
def write_run_scripts(case_dir, values):
    """One identical pipeline for every case. Never hand-type mpirun."""
    n = int(float(values.get("n_procs", 6)))
    solver = (values.get("solver") or "waamMhdFoam").strip()
    cid = values["case_id"]

    if n > 1:
        run = (f"runApplication decomposePar\n"
               f"runParallel {solver}\n"
               f"runApplication reconstructPar -latestTime\n")
    else:
        run = f"runApplication {solver}\n"

    (case_dir / "Allrun").write_text(f"""\
#!/bin/sh
cd "${{0%/*}}" || exit 1
. "$WM_PROJECT_DIR/bin/tools/RunFunctions"

# GENERATED BY generate_cases.py from caseMatrix.csv -- DO NOT EDIT BY HAND.
#   case   : {cid}
#   procs  : {n} ({values.get('decomp_method', 'scotch')})
#   p_rgh tolerance : {values.get('p_rgh_tol', '1e-07')}
# Numerics are frozen across a comparison set on purpose. Changing procs or
# decomposition between compared cases injects a perturbation that is
# indistinguishable from a physical effect.

if [ -d initial ] && [ ! -d 0 ]; then cp -r initial 0; fi
if [ -f 0/alpha.metal.orig ] && [ ! -f 0/alpha.metal ]; then
    cp 0/alpha.metal.orig 0/alpha.metal
fi

runApplication blockMesh
runApplication checkMesh
if [ -f system/setFieldsDict ]; then runApplication setFields; fi

{run}
# ParaView can read processor*/ directly (Case Type -> Decomposed Case),
# so reconstruct only what figures actually need.
""")
...
if [ -d initial ]; then rm -rf 0; fi
""")
    for s in ("Allrun", "Allclean"):
        (case_dir / s).chmod(0o755)
...
# Numerics that must be frozen across any set of cases you intend to compare.
# Physics may vary (that is the experiment); the numerical configuration may not.
FROZEN = ("n_procs", "decomp_method", "cell_size_m", "dt_s")

# A noise twin exists to measure the resolution limit, so it must differ from
# its reference in exactly one thing: the linear-solver tolerance.
TWIN_MAY_DIFFER = {"case_id", "enabled", "group", "notes", "p_rgh_tol",
                   "noise_twin_of"}


def audit(rows):
    """Enforce the noise-floor protocol mechanically instead of by memory."""
    problems, notes = [], []
    by_id = {r["case_id"].strip(): r for r in rows}
    live = [r for r in rows if truthy(r["enabled"])]

    # 1. numerics frozen within each comparison group
    # Key on (group, model): cases of different models are never compared to
    # each other, so their numerics are allowed to differ.
    groups = {}
    for r in live:
        key = (r.get("group", "").strip(), r["model"].strip().upper())
        groups.setdefault(key, []).append(r)
    for (g, model), members in sorted(groups.items()):
        if g in ("", "noisefloor", "meshstudy", "sensitivity") or len(members) < 2:
            continue  # these groups vary numerics deliberately
        for key in FROZEN:
            vals = {(r.get(key) or "").strip() for r in members}
            if len(vals) > 1:
                problems.append(
                    f"group '{g}' (model {model}): {key} differs across cases "
                    f"({', '.join(sorted(vals))}). Frozen numerics are required "
                    f"for a valid comparison.")

    # 2. every twin differs from its reference in exactly one field
    twins = [r for r in rows if (r.get("noise_twin_of") or "").strip()]
    for t in twins:
        ref_id = t["noise_twin_of"].strip()
        ref = by_id.get(ref_id)
        if ref is None:
            problems.append(f"{t['case_id']}: noise_twin_of '{ref_id}' is not a case")
            continue
        drift = [k for k in ref
                 if k not in TWIN_MAY_DIFFER
                 and (ref.get(k) or "").strip() != (t.get(k) or "").strip()]
        if drift:
            problems.append(
                f"{t['case_id']}: differs from {ref_id} in {', '.join(drift)}. "
                f"A twin must differ ONLY in p_rgh_tol.")
        if (t.get("p_rgh_tol") or "") == (ref.get("p_rgh_tol") or ""):
            problems.append(f"{t['case_id']}: same p_rgh_tol as {ref_id} -- no perturbation")

    # 3. groups that will be interpreted need a noise floor
    twinned = {(r.get("noise_twin_of") or "").strip() for r in twins if truthy(r["enabled"])}
    for (g, model), members in sorted(groups.items()):
        if g in ("", "noisefloor", "meshstudy", "sensitivity"):
            continue
        if not any(m["case_id"].strip() in twinned for m in members):
            notes.append(
                f"group '{g}' (model {model}) has no ENABLED noise twin. Differences "
                f"within it cannot be attributed to physics until one is run.")
...
def build_case(row, templates_dir, out_root, args, repo_root):
    ...
    write_decompose_par_dict(dest, values)
    write_run_scripts(dest, values)
...
def main():
    ...
    p.add_argument("--pairs", action="store_true", help="show noise-floor pairs and exit")
    p.add_argument("--ignore-audit", action="store_true",
                   help="proceed despite audit problems (not recommended)")
    args = p.parse_args()
    ...
    if problems and not args.ignore_audit:
        die("audit failed -- fix caseMatrix.csv, or pass --ignore-audit to proceed anyway")

    if args.pairs:
        twins = [r for r in rows if (r.get("noise_twin_of") or "").strip()]
        if not twins:
            die("no noise twins defined")
        print(f"\n{'REFERENCE':<24}{'TWIN':<30}{'PERTURBATION':<26}{'ON'}")
        print("-" * 88)
        for t in sorted(twins, key=lambda r: r["noise_twin_of"]):
            ref = t["noise_twin_of"].strip()
            refrow = next((r for r in rows if r["case_id"].strip() == ref), None)
            pert = f"p_rgh_tol {refrow['p_rgh_tol']} -> {t['p_rgh_tol']}" if refrow else "?"
            on = "yes" if truthy(t["enabled"]) else "no"
            print(f"{ref:<24}{t['case_id']:<30}{pert:<26}{on}")
        print("\nRun each pair, extract every reported quantity from both, and take")
        print("the spread as the noise floor. Effects below it are not results.\n")
        return
